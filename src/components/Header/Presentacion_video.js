...
export default function Presentacionvideo(){
  useEffect(()=>{
    ...
    return(
        <header id='inicio' className='h-[200px] sm:h-[150px] lg:h-[190px] xl:h-[200px] 2xl:h-[200px]'>
            <figure>
            <img loading='lazy' className='fill-current w-full h-[200px] xl:hidden md:hidden sm:block lg:block 2xl:hidden' alt='imgfondo' src='/media/fondo.png'/>
            </figure>
            <video preload='auto' autoPlay loop className='hidden  sm:hidden sm:h-[150px] sm:w-full xl:block xl:w-full xl:h-[200px] md:block md:h-[150px] lg:hidden lg:h-[190px] 2xl:block 2xl:h-[200px]'>
              <source src="https://res.cloudinary.com/dengeltec/video/upload/v1663273125/samples/den/portafoli2000px_lcfxbw.mp4"   type='video/mp4'/>
            </video>
